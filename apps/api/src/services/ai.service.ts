...
export const AIService = {
	async analyzeDocument(
		...
		// Document analysis runs locally — no Python service call needed.
		// The rule-based fallback handles all validation cases (expiry, fraud, mismatch, etc.)
		...
		if (lowerUrl.includes("mismatch")) {
			return {
				...
				validation: {
					passed: false,
					reason: "The uploaded file does not match the required document type.",
					issueType: "mismatch",
				},
			};
		}
		...
		if (documentText && documentType) {
			const expirationCheck = checkDocumentExpiration(documentType, documentText);
			if (expirationCheck.isExpired) {
				return {
					confidence: 0.95,
					validation: {
						passed: false,
						reason: expirationCheck.reason || "Document is expired. Please upload the renewed version.",
						issueType: "expired",
					},
				};
			}
		...
		if (lowerUrl.includes("expired")) {
			return {
				...
				validation: {
					passed: false,
					reason: "Your document is expired. Please upload the renewed version.",
					issueType: "expired",
				},
			};
		}
		...
		if (
			...
			return {
				confidence: 0.9,
				extractedText: "Business Name: ACME Solutions Inc.\nTIN: 12-3456789\nLicense: BL-987654",
				summary: "Document contains business information that may conflict with other documents.",
